"""Find charge-charge interactions in an input structure"""
...
def compute_pairwise_energies(
    ...
    max_dist_minus_sigma=0.5, charged_aas=['GLU', 'ASP', 'ARG', 'LYS']
):
    """
    ...
    energies_dict = {
        ...
        for key in [
            ...
            'atom_j_is_acceptor', 'atom_j_is_polar_hydrogen',
    ...
    for res_j_n in range(1, pose.size()+1):
        ...
        res_j_name = res_j.name3()
        if res_j_name not in charged_aas:
            continue
        for atom_j_n in range(1, res_j.natoms()+1):
            ...
            energies_dict['res_j_name'].append(res_j_name)
            ...
            energies_dict['atom_j_is_acceptor'].append(
                atom_type_j.is_acceptor()
            )
            energies_dict['atom_j_is_polar_hydrogen'].append(
                atom_type_j.is_polar_hydrogen()
            )
    ...
    energies_df['atom_i_is_acceptor'] = atom_type_i.is_acceptor()
    energies_df['atom_i_is_polar_hydrogen'] = atom_type_i.is_polar_hydrogen()
...
def main():
    ...
    parser.add_argument("--pdb_dir", help="the path to a directory of input PDB files")
    ...
    # Cycle over input PDBs and find interactions
    ...
    for pdb in pdbs:
        ...
        charged_aas = ['GLU', 'ASP', 'ARG', 'LYS']
        max_dist = 6
        max_dist_minus_sigma = 100
        for res_i_n in range(1, pose.size()+1):
            res_i = pose.residue(res_i_n)
            res_i_name = res_i.name3()
            if res_i_name not in charged_aas:
                continue
            for atom_i_n in list(range(1, res_i.natoms()+1)):
                ...
                df['pdb_file'] = pdb
                df['pdb'] = os.path.basename(pdb)
                dfs.append(df)
            
    # Make a single dataframe with all atom pair energies and
    # subset to desired charge-charge interactions
    ...
    energies_df = energies_df[
        ...
        (
            ((energies_df['atom_i_is_acceptor']) & (energies_df['atom_j_is_polar_hydrogen'])) |
            ((energies_df['atom_j_is_acceptor']) & (energies_df['atom_i_is_polar_hydrogen']))
        )
    ]
