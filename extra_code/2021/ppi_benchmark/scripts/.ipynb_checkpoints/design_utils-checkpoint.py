...
def WriteSbatchFile(
    ...
    memory='2g', array=None, group_size=None, n_commands=None, write_out_err=True,
    ):
    """
...
def extract_pdbs_from_silent_file(
    working_dir, silent_file, extra_args=[], tags=[], output_dir=False, print_cmd=False
):
    cmd = ' '.join([
        '/home/haddox/Rosetta/main/source/bin/extract_pdbs.linuxgccrelease',
        '-database /home/haddox/Rosetta/main/database/',
        '-in:file:silent {0}'.format(silent_file),
        #'-in:file:silent_struct_type binary',
        '-silent_read_through_errors',
        '-crystal_refine'
    ])
    if len(tags) > 0:
        cmd += ' -in:file:tags {0}'.format(' '.join(tags))
    if output_dir:
        cmd += ' -out:prefix {0}'.format(output_dir)
    if extra_args:
        for arg in extra_args:
...
def call_rosettascripts_xml(
    rosettapath, databasepath, xml_file, relaxscript, pdbs, weights_file,
    output_dir, scores_file_prefix, extra_script_vars=None, extra_args=None,
    flags_file=None, submit_sbatch_job=True, queue_type='medium', memory='2g',
    sbatch_group_size=None, subsample_pdbs=False
    ):
    """
    Call a RosettaScripts XML with given inputs, submitting job to cluster
